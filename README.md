...
The initial testing scores showed on average an 60-65% total accuracy score with most notable mistakes occuring in complex text contents such as scanned documents and polytonic accents. For this, the Tesseract model was experimentally trained using in-house data extracted from scraped PDFs, consisting of paired image files (.tif) and their corresponding ground-truth text files (.gt.txt). 

However, this approach did not lead to a meaningful improvement in accuracy. The results suggested that the remaining errors were largely due to the architectural limitations of traditional OCR systems, particularly when handling complex layouts, degraded scans, and polytonic Greek text.

**2. [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR)** 

The initial testing showed a slightly better average accuracy of 65-70%, with the same frequent misrecognition patterns as Tesseract. Compared to Tesseract, PaddleOCR handled noisy or varied fonts slightly better, but it struggled to maintain character-level accuracy and proper accents. Unlike Tesseract which is CPU-bound, PaddleOCR uses neural detection and recognition models, which makes it more flexible for real-world document layouts and more suitable for GPU-accelerated batch processing, thus giving it stronger scalability path for large OCR workloads. No additional custom training was performed for PaddleOCR during this stage so results are its out-of-the-box performance.

**3. [RapidOCR](https://github.com/RapidAI/RapidOCR) & [onnx](https://github.com/onnx/onnx)**

The initial testing demonstrated identical results to PaddleOCR, with comparable accuracy and similar remaining weaknesses on accented Greek and polytonic text. Its advantage was mainly architectural. By running OCR models through ONNX Runtime, RapidOCR provided a lighter inference path, better portability, reduced dependency on the full PaddlePaddle stack, and support for optimized CPU or GPU execution. Combined with Docling’s native RapidOCR support, this made it a more practical option for scalable GlossAPI integration. ike PaddleOCR, no additional training was performed at this stage.
